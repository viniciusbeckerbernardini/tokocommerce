    ...
    $page->setTpl("payment-pagseguro",
        [
            ...
            "phone"=>[
                ...
                "nrPhone"=>substr($order->getnrphone(),2,strlen($order->getnrphone()))
            ]
        ]);
});
...
$app->get("/boleto/:idorder",function (int $idorder){
    ...
    $order->get($idorder);
    $vltotal = str_replace(",","",$order->getvltotal());
    $dias_de_prazo_para_pagamento = 10;
    $taxa_boleto = 5.00;
    $data_venc = date("d/m/Y", time() + ($dias_de_prazo_para_pagamento * 86400));  // Prazo de X dias OU informe data: "13/04/2006";
    $valor_cobrado = $vltotal; // Valor - REGRA: Sem pontos na milhar e tanto faz com "." ou "," ou com 1 ou 2 ou sem casa decimal
    ...
    $dadosboleto["endereco2"] = $order->getdesdrict()." - ".$order->getdescity();
...
$app->post("/profile/change-password",function(){
    ...
    if($_POST['current_pass'] === $_POST['new_pass']){
        User::setMsgError("A senha deve ser diferente da atual.");
        header("Location: /profile/change-password");
        exit();
    }
